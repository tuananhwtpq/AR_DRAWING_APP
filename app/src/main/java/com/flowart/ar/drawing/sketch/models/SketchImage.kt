...
class SketchImage(private val originalBitmap: Bitmap) {
    ...
    fun getImageAs(effectType: SketchEffect, thickness: Int): Bitmap {
        ...
        val result = when (effectType) {
            ...
            SketchEffect.ORIGINAL_TO_SOFT_COLOR_SKETCH -> toSoftColorSketch(
                downscaledBitmap,
                thickness
            )

            SketchEffect.GRAY_TO_SKETCH -> toSketch(toGrayScale(downscaledBitmap), thickness)
            SketchEffect.GRAY_TO_COLORED_SKETCH -> toColoredSketch(
                toGrayScale(downscaledBitmap),
                thickness
            )

            SketchEffect.GRAY_TO_SOFT_SKETCH -> toSoftSketch(
                toGrayScale(downscaledBitmap),
                thickness
            )

            SketchEffect.GRAY_TO_SOFT_COLOR_SKETCH -> toSoftColorSketch(
                toGrayScale(downscaledBitmap),
                thickness
            )
    ...
    private fun toStrokeOnly(bitmap: Bitmap, thickness: Int): Bitmap {
        ...
        val resultBitmap =
            Bitmap.createBitmap(matEdges.cols(), matEdges.rows(), Bitmap.Config.ARGB_8888)
    ...
    private fun toSketch(bitmap: Bitmap, thickness: Int): Bitmap {
        ...
        val blurredBitmap =
            Bitmap.createBitmap(matBlurred.cols(), matBlurred.rows(), Bitmap.Config.ARGB_8888)
